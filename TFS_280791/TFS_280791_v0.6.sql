...
SELECT
	...
	/* Регуоярные выражения REPLACE не хочет принимать
	/* https://docs.microsoft.com/ru-ru/sql/ssms/scripting/search-text-with-regular-expressions?view=sql-server-2017 */
	(
	   REPLACE(
	      /*REPLACE(
		     REPLACE(
...
SELECT DISTINCT
	...
	CONCAT (region_id, ',', traffic, ',', operator, ',', DestTrunkGroup) description,
...
WHERE circuit_id NOT LIKE '%[А-Яа-я]%'
/*WHERE convert(varchar, circuit_id) = ''*/
/* Ничего не помогает убрать пробел
AND convert(varchar, circuit_id) = ''
AND circuit_id LIKE CAST('% %' as text)
AND datalength(circuit_id)<>0
AND circuit_id <> char(13)
AND circuit_id > ''
circuit_id NOT LIKE '% %'
circuit_id NON EMPTY 
NULLIF(circuit_id, '') IS NOT NULL 
len(circuit_id) > 0    
AND circuit_id NOT LIKE ''
*/
