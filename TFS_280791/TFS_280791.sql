...
NewCase (SystemId, ServiceId, SrvEngId, ActiveId, CaseId, DateLast, ClosureCode, Reopen) AS (
   SELECT 	  
	  ...
	  Cases.ServiceItemId,        /* Услуги  */
	  ServiceEngineer.EngineerId, /* Группа сервисных инженеров */
	/*qG.qUid,                       Сервисные инженеры */
	  Servic.StatusId,            /* Признак = Статус Услуги (Предоставляется / Не предоставляется) */
	  Cases.Id,                   /* Кол-во обращений */
	  Cases.RegisteredOn,         /* Дата последнего обращения */
	  Cases.ClosureCodeId,        /* Сколько автоматом */	  
	/*CaseLifecycle.StatusId         Переоткрыто */
	  CaseLife.StatusId              Переоткрыто
