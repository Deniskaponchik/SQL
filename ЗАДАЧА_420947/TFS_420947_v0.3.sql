...
left join [T2RU-BPMDB-05\BPMONLINE].[BPMonline7].[dbo].[SysAdminUnit] sau1 on sau1.id = c.UsrSolvedGroupId  --Группа ответственных
--left join SysAdminUnit sau2 on sau2.id = c.UsrResolvedGroupId --Ещё какая-то группа ответственных

left join [T2RU-BPMDB-05\BPMONLINE].[BPMonline7].[dbo].[UsrCaseChangeHistory] ucch1 
  on ucch1.UsrCaseId = c.Id and ucch1.NewStatusId = 'F063EBBE-FDC6-4982-8431-D8CFA52FEDCF' --переоткрыто

--Поиск заявок, выполненных Автоматически (идея взята: https://wiki.tele2.ru/pages/viewpage.action?pageId=38538972)
left join [T2RU-BPMDB-05\BPMONLINE].[BPMonline7].[dbo].[UsrCaseChangeHistory] ucch2 
   on ucch2.UsrCaseId = c.Id 
  and ucch2.NewStatusId = 'AE7F411E-F46B-1410-009B-0050BA5D6C38'  --Решено
  and ucch2.OldStatusId = '49284152-9CA5-4006-B97C-A6D4382D7FAD'  --Назначено
  and ucch2.NewGroupId = c.UsrSolvedGroupId

--Это рабочее для Сервионики. Если понадобится, можно включить.
--left join [T2RU-BPMDB-05\BPMONLINE].[BPMonline7].[dbo].[UsrSpecificationInCase] usic1
--on usic1.UsrCaseId = c.id and usic1.SpecificationId = '558B3F8B-FA19-4D62-915B-4326B7C087AC'  --Фактическая дата начала
--left join [T2RU-BPMDB-05\BPMONLINE].[BPMonline7].[dbo].[UsrSpecificationInCase] usic2 
--on usic2.UsrCaseId = c.id and usic2.SpecificationId = '10057311-E073-4347-9B21-1B8DC032FA73'  --Фактическая дата Завершения
--left join [T2RU-BPMDB-05\BPMONLINE].[BPMonline7].[dbo].[UsrSpecificationInCase] usic3 
--on usic3.UsrCaseId = c.id and usic3.SpecificationId = '7E0F18B6-3857-463C-8ABF-A38486FCB2CF'  --Назначенный инженер
  left join [T2RU-BPMDB-05\BPMONLINE].[BPMonline7].[dbo].[UsrSpecificationInCase] usic4 
  on usic4.UsrCaseId = c.id and usic4.SpecificationId = '9957AB2C-A353-49BF-A6EB-9968966C4BE8'  --Регион из Перечня параметров
--left join [T2RU-BPMDB-05\BPMONLINE].[BPMonline7].[dbo].[UsrSpecificationInCase] usic5 
--on usic5.UsrCaseId = c.id and usic5.SpecificationId = '4E96E7C5-60FC-4D0C-845E-7FD9D3D5200E'  --Адрес офиса
...
--select GetDate()
  and c.FirstSolutionProvidedOn BETWEEN cast('2022-01-01' as datetime) and GetDate()  --Константин
--and c.FirstSolutionProvidedOn > '2022-01-01 00:00:00.000' --Мой вариант
--and c.ClosureDate BETWEEN '2021-12-01 00:00:00' and '2021-12-31 23:59:59'
--and c.ClosureDate BETWEEN '2022-01-01 00:00:00.000' and GetDate()  -MAIN
