...
--СТАТУС: РАБОЧЕЕ
--РЕАЛИЗАЦИЯ: Без параметра Регион в Перечне параметров
--ПРОБЛЕМЫ: 
...
--  , urc.name 'Регион'
  ...
  --, ucch.NewStatusId 'Решено с первого раза'
	...
	, c.UsrIsScriptExecuted 'АвтоВыполнение'
...
left join (
	select UsrCaseid, StringValue
	from UsrSpecificationInCase
	where SpecificationId = 'ЖДЁМ СОЗДАНИЯ НОВОГО СПРАВОЧНИКА РЕГИОНОВ'  --Регион из перечня Параметров
