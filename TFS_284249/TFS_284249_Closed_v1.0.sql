...
OldStatusOfIncidentId	uniqueidentifier,
NewStatusOfIncidentId	uniqueidentifier,
ChangeDate	datetime2(3),
NotChangingTime	nvarchar(50),
...
--SELECT *
--FROM OPENROWSET('SQLNCLI', 'Server='+@DBserver+'\BPMOnline;UID=helper;PWD=b4vax',
...
--Выборка из CounterInIncident с нужными заявками
INSERT INTO @CII (OldGroupId,NewGroupId,OldStatusOfIncidentId,NewStatusOfIncidentId,ChangeDate,NotChangingTime,ServiceRequestId)
	SELECT OldGroupId,NewGroupId,OldStatusOfIncidentId,NewStatusOfIncidentId,ChangeDate,NotChangingTime,ServiceRequestId 
...
INSERT INTO @GroupTimeCii0 (ServiceRequestId,OldGroupId,MaxChangeDate)
	SELECT ServiceRequestId, OldGroupID, MAX(ChangeDate)
	FROM @CII
...
INSERT INTO @GroupTimeCii1 (ServiceRequestId,OldGroupId,MaxChangeDate)
	SELECT DISTINCT 
		  cii.ServiceRequestId ServiceRequestId
		, cii.OldGroupId OldGroupId
		, case
			 when GroupTimeCII0.OldGroupId = cii.NewGroupId then NULL
			 else GroupTimeCII0.MaxChangeDate
		END MaxChangeDate
	FROM @CII cii
	LEFT JOIN @GroupTimeCII0 GroupTimeCII0 ON cii.ServiceRequestId = GroupTimeCII0.ServiceRequestId 
	      AND cii.OldGroupID = GroupTimeCII0.OldGroupID
	WHERE 
			    cii.OldGroupId = GroupTimeCII0.OldGroupID
			AND cii.ChangeDate = GroupTimeCII0.MaxChangeDate --останется только одна строка
