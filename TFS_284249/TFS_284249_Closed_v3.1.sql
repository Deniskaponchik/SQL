...
--INSERT INTO #GroupTimeCii2 (ServiceRequestId,OldGroupId,MaxNotChangingTime,MinNotChangingTime,SumNotChangingTime)
	SELECT    ServiceRequestId, OldGroupId, CreatedOn, NotChangingTime
	...
	into #GroupTimeCii2
	from #CII
	where CreatedOn is not NULL             -- этого условия не было до изменений
...
,GroupTimeCii.NotChangingTime /*SumNotChangingTime*/ 'Время обработки группой (мин.)'
...
LEFT JOIN ( --'Общее время обработки (мин.)'
	SELECT ServiceRequestId
		, SUM(NotChangingTime) /*(SumNotChangingTime)*/ SumFullNotChangingTime
	FROM #GroupTimeCii2
...
OUTER APPLY (
	...
	--, MaxNotChangingTime, MinNotChangingTime
	, NotChangingTime                    --SumNotChangingTime
...
ORDER BY SR.Number, GroupTimeCII.CreatedOn
