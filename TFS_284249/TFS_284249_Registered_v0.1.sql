...
/*,COALESCE(vwIOGT.SumOnGroupTime, 0) 'Общее время обработки (мин.)'*/
,SumFullCII.SumFullNotChangingTime 'Общее время обработки (мин.)'
/*,COALESCE(vwIOGT.MaxOnGroupTime, 0) 'Максимальное время обработки группой (мин.)'*/
,GroupTimeCII.MaxNotChangingTime 'Максимальное время обработки группой (мин.)'
/*,COALESCE(vwIOGT.MinOnGroupTime, 0) 'Минимальное время обработки группой (мин.)' */
,GroupTimeCII.MinNotChangingTime 'Минимальное время обработки группой (мин.)'
/*,COALESCE(SAU.Name, '') 'Группа'*/
,GroupTimeCII.NewGroupId 'Группа'
/*,vIOGT.ChangeDate 'Дата перевода на группу'*/
,GroupTimeCII.MaxChangeDate 'Дата перевода на группу'
/*,COALESCE(vIOGT.OnGroupTime, 0) 'Время обработки группой (мин.)'*/
...
/* LEFT JOIN (
	SELECT SUM(OnGroupTime) as SumOnGroupTime
		  ,MAX(OnGroupTime) as MaxOnGroupTime
	      ,MIN(OnGroupTime) as MinOnGroupTime
	      ,ServiceRequestId
...
/* !!! РАСКОММЕНТИРОВАТЬ В КОНЦЕ НА ПРОДУКТОВОЙ БАЗЕ !!! */
