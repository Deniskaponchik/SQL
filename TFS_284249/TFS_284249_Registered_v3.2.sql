--С использованием Declare. Код завис на час без возможности отмены, потом SMS завис полностью. Повторять не рекомендуется
...
CreatedOn	datetime2(3),
ServiceRequestId	uniqueidentifier,
cii0_OldGroupId	uniqueidentifier, cii1_OldGroupId	uniqueidentifier, cii2_OldGroupId	uniqueidentifier,
cii0_NewGroupId	uniqueidentifier, cii1_NewGroupId	uniqueidentifier, cii2_NewGroupId	uniqueidentifier,
cii0_NotChangingTime int, cii1_NotChangingTime int, cii2_NotChangingTime int,
...
INSERT INTO @CII (RowNumber, CreatedOn, ServiceRequestId
	,cii0_OldGroupId ,cii1_OldGroupId ,cii2_OldGroupId
	,cii0_NewGroupId, cii1_NewGroupId, cii2_NewGroupId
    ,cii0_NotChangingTime,cii1_NotChangingTime,cii2_NotChangingTime
    ,RightNotChangingTime)
SELECT 
   cii1.RowNumber, cii1.CreatedOn,cii1.ServiceRequestId 
   --cii1.NewStatusOfIncidentId,cii1.OldGroupId,cii1.NewGroupId --,cii0.rownumber,cii1.RowNumber, cii2.RowNumber
  ,cii0.OldGroupId,cii1.OldGroupId,cii2.OldGroupId
  ,cii0.NewGroupId,cii1.NewGroupId,cii2.NewGroupId
  ,cii0.NotChangingTime,cii1.NotChangingTime,cii2.NotChangingTime
...
DECLARE @CountCii0 INT, @RowNumber INT, @PlusNotChangingTime INT
SELECT @CountCii0 = (SELECT COUNT(*) FROM @cii0), @RowNumber = 1
WHILE @Countcii0 > 0
BEGIN	
--SET @Countcii0  = @Countcii0 - 1 SELECT  @CountCii0 end

UPDATE @cii 
SET RightNotChangingTime = @RowNumber
...
SELECT * FROM @cii
	...
	/*
	DECLARE @CountCii0 INT, @RowNumber INT
	, @cii0_OldGroupId uniqueidentifier, @cii1_OldGroupId uniqueidentifier, @cii2_OldGroupId uniqueidentifier
	, @cii0_NewGroupId uniqueidentifier, @cii1_NewGroupId uniqueidentifier, @cii2_NewGroupId uniqueidentifier
	, @cii0_ServiceRequestId uniqueidentifier, @cii1_ServiceRequestId uniqueidentifier, @cii2_ServiceRequestId uniqueidentifier
SELECT @CountCii0 = (SELECT COUNT(*) FROM #cii0),@RowNumber = 1
	, @cii0_OldGroupId = NULL, @cii1_OldGroupId = NULL, @cii2_OldGroupId = NULL
	, @cii0_NewGroupId = NULL, @cii1_NewGroupId = NULL, @cii2_NewGroupId = NULL
	, @cii0_ServiceRequestId = NULL, @cii1_ServiceRequestId = NULL, @cii2_ServiceRequestId = NULL

WHILE @Countcii0 > 0
BEGIN	
	SELECT OldGroupId,NewGroupId,NewStatusOfIncidentId,CreatedOn,NotChangingTime,ServiceRequestId
	INTO #cii
	FROM #cii0
	WHERE RowNumer = @RowNumber
	SET @cii0_ServiceRequestId = case 
		when @cii0_ServiceRequestId IS NULL AND @cii1_ServiceRequestId IS NULL AND @cii2_ServiceRequestId IS NULL then NULL ---
		when @cii0_ServiceRequestId IS NULL then ServiceRequestId
		when @cii2_ServiceRequestId IS NULL then ServiceRequestId
	SET @cii1_ServiceRequestId = case 
		when @cii0_ServiceRequestId IS NULL AND @cii1_ServiceRequestId IS NULL AND @cii2_ServiceRequestId IS NULL then ServiceRequestId
		when @cii1_ServiceRequestId IS NULL then ServiceRequestId
		when @cii2_ServiceRequestId IS NULL then ServiceRequestId
	SET @cii2_ServiceRequestId = case 
		when @cii0_ServiceRequestId IS NULL then ServiceRequestId
		when @cii1_ServiceRequestId IS NULL then ServiceRequestId
		when @cii2_ServiceRequestId IS NULL then ServiceRequestId
	SET @cii0_OldGroupId = case 
		when @cii0_OldGroupId IS NULL AND @cii1_OldGroupId IS NULL AND @cii2_OldGroupId IS NULL then NULL ---
		when @cii0_OldGroupId IS NULL then OldGroupId
		when @cii2_OldGroupId IS NULL then OldGroupId
	SET @cii1_OldGroupId = case 
		when @cii0_OldGroupId IS NULL AND @cii1_OldGroupId IS NULL AND @cii2_OldGroupId IS NULL then OldGroupId
		when @cii1_OldGroupId IS NULL then OldGroupId
		when @cii2_OldGroupId IS NULL then OldGroupId
	SET @cii2_OldGroupId = case 
		when @cii0_OldGroupId IS NULL then OldGroupId
		when @cii1_OldGroupId IS NULL then OldGroupId
		when @cii2_OldGroupId IS NULL then OldGroupId
