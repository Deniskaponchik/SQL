...
INSERT INTO @CII0 (OldGroupId,NewGroupId,NewStatusOfIncidentId,CreatedOn,NotChangingTime,ServiceRequestId)
	SELECT OldGroupId,NewGroupId
	--,NewStatusOfIncidentId
	,case
		when NotChangingTime = '' then NULL --НЕ ПРОСТО ТАК СДЕЛАНО
		else NewStatusOfIncidentId
	END
	,CreatedOn
	...
	,case
		when NotChangingTime = '' then '0 д. 0 ч. 0 мин.' --НЕ ПРОСТО ТАК СДЕЛАНО
	...
	WHERE
	    ...
	    --ServiceRequestId = '1D9B1432-33BF-45E7-9EEE-44B78D2C093D'
		--AND NotChangingTime != ''


--Выборка из Cii0 нужными заявками. CounterInIncident скрещивается с самой собой несколько раз
...
,case --джойним ПРОШЛУЮ, НАСТОЯЮЩУЮ и БУДУЩУЮ строки
...
END RightNotChangingTime
  ...
  /*,cii0.ServiceRequestId,cii1.ServiceRequestId, cii2.ServiceRequestId
  ,cii0.NotChangingTime,cii1.NotChangingTime, cii2.NotChangingTime
  ,cii0.OldGroupId,cii1.OldGroupId,cii2.OldGroupId
  ,cii0.RowNumber,cii1.RowNumber,cii2.RowNumber*/
FROM ( --НАСТОЯЩАЯ строка
...
INSERT INTO @GroupTimeCii1 (ServiceRequestId,OldGroupId,MaxCreatedOn)
	...
	WHERE --cii3.OldGroupId = GroupTimeCII0.OldGroupID AND 
		  cii3.CreatedOn = GroupTimeCII0.MaxCreatedOn --останется только одна строка
