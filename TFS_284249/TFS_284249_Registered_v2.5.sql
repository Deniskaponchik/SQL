...
--ПРОБЛЕМЫ: Эффективно ли сравнивается значение с NULL в строке 56 ?
...
--, ISNULL(OldGroupId) OldGroupId
  , ISNULL(CONVERT(UNIQUEIDENTIFIER,OldGroupId),0x0)  OldGroupId
...
--order by CreatedOn
  ...
  SELECT ServiceRequestId, NewGroupId, SUM(NotChangingTime) as SumNotChangingTime
  into #cii1
  from #cii0
  where
		(
  ...
  GROUP BY ServiceRequestId, NewGroupId --OldGroupID
...
OUTER APPLY (
	...
	FROM
	#cii1 cii1
	left join #cii2 cii2 on cii1.ServiceRequestId = cii2.servicerequestid and cii1.NewGroupId = cii2.NewGroupId
	LEFT JOIN SysAdminUnit SAU WITH(NOLOCK) ON SAU.Id = Cii1.NewGroupId
