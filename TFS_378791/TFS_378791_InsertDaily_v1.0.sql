...
--СТАТУС: На тестовом сервере нет прав на запись для создания и записи какой-нибудь таблицы
--РЕАЛИЗАЦИЯ: Запускать на сервере ANALYTICS
--ПРОБЛЕМЫ: В каком виде необходимо указывать БД прода ?


USE ANALYTICSS;

--Declare @BPM5_SLA_TFS360841_4PiR Table(
--	 TT nvarchar(250)
...
--Кусок кода от Годовой выгрузки
--Declare @Date datetime2(3) = GetDate()
--WHILE (CONVERT(date,@Date,101) != '2022-01-01')  --@Date != '2022-05-01 00:00:00'
--BEGIN
--	SET @Date = dateadd(day,-1,@Date)
	...
	--SELECT *
	--FROM OPENROWSET('SQLNCLI', 'Server=t2ru-bpmdb-read\BPMOnline;UID=;PWD=;',
	...
	--Таблица с необходимыми заявками, на которую в последствие ссылаются все остальные
		...
		FROM [t2ru-bpmdb-read\BPMOnline].[BPMonline].[dbo].[ServiceRequest]
		WHERE 
		  ClosureDate >= dateadd(day,-1,getdate())  --MAIN
		--ClosureDate >= @Date
		--ClosureDate > '2021-09-24 00:00:00' and ClosureDate < '2021-09-24 23:59:59' --FASTER
		--ClosureDate BETWEEN '2021-09-20 00:00:00' and '2021-09-24 23:59:59'
		--ClosureDate BETWEEN '2021-01-01 00:00:00' and '2021-07-08 23:59:59' -- SELECT GetDate()
	...
	FROM [t2ru-bpmdb-read\BPMOnline].[BPMonline].[dbo].[CounterInIncident]
	WHERE
	...
	--where spfsr.Name IS NULL
	ORDER BY SR.Number


	--) --OpenRowSet END
	--select @BPM5_SLA_TFS360841_4PiR
	--select @param = '@Date datetime2(3)'
	--EXEC sp_executesql @BPM5_SLA_TFS360841_4PiR, @param, @Date=@Date
--END  -- End of Loop
