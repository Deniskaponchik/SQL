...
--ПРОБЛЕМЫ: Отрабатывает медленно

--USE ANALYTICSS;

--Declare @Date datetime2(3) = GetDate()  --Моё
--Declare @end datetime2(3) = GetDate()  --Моё
--declare @end   datetime2(3) = dateadd(HH,+3,getdate()); 
--select dateadd(dd, datediff(dd, 0, getdate()) - 1, 0) as yesterday
--select dateadd(HH,3,dateadd(dd, datediff(dd, 0, getdate()) - 1, 0)) as yesterday3
  Declare @Start datetime2(3) = dateadd(HH,3,dateadd(dd, datediff(dd, 0, getdate()) - 1, 0))
--SELECT DATEADD(day, DATEDIFF(day, '19000101', GETDATE()), '19000101');
--SELECT dateadd(HH,3,DATEADD(day, DATEDIFF(day, '19000101', GETDATE()), '19000101')) as today3
  Declare @End datetime2(3) = dateadd(HH,3,DATEADD(day, DATEDIFF(day, '19000101', GETDATE()), '19000101'))
...
--BEGIN
	...
	--Таблица с необходимыми заявками, на которую в последствие ссылаются все остальные
		...
		--ClosureDate BETWEEN '2021-10-23 00:00:00' and '2021-10-23 23:59:59'
		--ClosureDate BETWEEN '2022-12-02 00:00:00' and '2022-12-02 23:59:59'
		  ClosureDate BETWEEN @start and @end  --МОЁ
		--dateadd(HH,3,ClosureDate) BETWEEN '2022-12-02 00:00:00' and '2022-12-02 23:59:59' --SLOWLY!!!
		--dateadd(HH,3,ClosureDate) BETWEEN @start and @end  --МОЁ
