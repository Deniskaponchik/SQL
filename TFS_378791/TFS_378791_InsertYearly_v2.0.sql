...
--СТАТУС: 
--РЕАЛИЗАЦИЯ: Связанные серверы + Подсчёт времени NotChangingTime в CounterInIncident через скрещивание таблицы с самой собой 
--ПРОБЛЕМЫ: 
...
BEGIN
	...
	SELECT 
	  ...
	  ,cii1.OldGroupId
	  ,cii1.NewGroupId
	  ,cii1.CreatedOn --Время уже московское в поле
	...
	FROM (
		...
		,ServiceRequestId ,OldGroupId ,NewGroupId ,NotChangingTime ,CreatedOn
		FROM #cii00
	) cii1
