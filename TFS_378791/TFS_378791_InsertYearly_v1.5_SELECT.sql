...
--select dateadd(HH,3,dateadd(dd, datediff(dd, 0, getdate()) - 1, 0)) as yesterday3
  Declare @Start datetime2(3) = dateadd(HH,3,dateadd(dd, datediff(dd, 0, getdate()) - 1, 0)) --Yesterday 00:00 +3 HH
...
--SELECT dateadd(HH,3,DATEADD(day, DATEDIFF(day, '19000101', GETDATE()), '19000101')) as today3
  Declare @End datetime2(3) = dateadd(HH,3,DATEADD(day, DATEDIFF(day, '19000101', GETDATE()), '19000101')) --объявляю, но не использую
...
--WHILE (@start != @end)      --Константин
  WHILE (@start != '2021-12-31 03:00:00.000')
...
BEGIN
  ...
  --,SR.ClosureDate AS 'Close_Date'
	,dateadd(hh,+3,sr.ClosureDate) Close_Date
  ...
  --,Sla_Time_minutes - cii1.Fact_work_time_minutes 'SLA_delta_minutes'
	...
	--Declare @Start datetime2(3) = dateadd(HH,3,dateadd(dd, datediff(dd, 0, getdate()) - 1, 0))
      SET @start = dateadd(day,-1,@start)  -- минус 1 день. Идём с конца года в начало
	  SET @End   = dateadd(day,-1,@end)    --объявляю, но не использую
	--select @start
